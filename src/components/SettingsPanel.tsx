      ...
      <Tabs defaultValue="api" className="w-full">
        ...
        <TabsContent value="api" className="space-y-3 mt-0">
          <div className="config-field">
            ...
            <Input type="text" placeholder="Partner ID / API Key" value={config.apiCredentials.apiKey} onChange={e => onUpdateApiCredentials({
            apiKey: e.target.value
          })} className="h-9 text-sm" />
          </div>
          <div className="config-field">
            <Label className="config-label">API Secret</Label>
            <Input type="password" placeholder="Partner Secret" value={config.apiCredentials.apiSecret} onChange={e => onUpdateApiCredentials({
            apiSecret: e.target.value
          })} className="h-9 text-sm" />
          </div>
          <div className="config-field">
            <Label className="config-label">Portokasse Login</Label>
            <Input type="text" placeholder="Email / Username" value={config.apiCredentials.portokasseLogin} onChange={e => onUpdateApiCredentials({
            portokasseLogin: e.target.value
          })} className="h-9 text-sm" />
          </div>
          <div className="config-field">
            <Label className="config-label">Portokasse Password</Label>
            <Input type="password" placeholder="Password" value={config.apiCredentials.portokassePassword} onChange={e => onUpdateApiCredentials({
            portokassePassword: e.target.value
          })} className="h-9 text-sm" />
          </div>
        </TabsContent>
        ...
        <TabsContent value="sender" className="space-y-3 mt-0">
          <div className="config-field">
            ...
            <Input type="text" placeholder="Full Name" value={config.senderAddress.name} onChange={e => onUpdateSenderAddress({
            name: e.target.value
          })} className="h-9 text-sm" />
          </div>
          <div className="config-field">
            <Label className="config-label">Company (optional)</Label>
            <Input type="text" placeholder="Company Name" value={config.senderAddress.company || ''} onChange={e => onUpdateSenderAddress({
            company: e.target.value
          })} className="h-9 text-sm" />
          </div>
          <div className="config-field">
            <Label className="config-label">Street & Number</Label>
            <Input type="text" placeholder="Musterstraße 123" value={config.senderAddress.street} onChange={e => onUpdateSenderAddress({
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="config-field">
              ...
              <Input type="text" placeholder="12345" value={config.senderAddress.postalCode} onChange={e => onUpdateSenderAddress({
              postalCode: e.target.value
            })} className="h-9 text-sm" />
            </div>
            <div className="config-field col-span-2">
              <Label className="config-label">City</Label>
              <Input type="text" placeholder="Berlin" value={config.senderAddress.city} onChange={e => onUpdateSenderAddress({
              city: e.target.value
            })} className="h-9 text-sm" />
            </div>
          </div>
          <div className="config-field">
            ...
            <Input type="text" placeholder="DE" value={config.senderAddress.country} onChange={e => onUpdateSenderAddress({
        ...
        <TabsContent value="printer" className="space-y-3 mt-0">
          <div className="config-field">
            ...
            <Select value={config.printerConfig.paperFormat} onValueChange={value => onUpdatePrinterConfig({
            paperFormat: value
          })}>
              ...
              <SelectContent>
                {PAPER_FORMATS.map(format => <SelectItem key={format.id} value={format.id}>
                    {format.name}
                  </SelectItem>)}
              </SelectContent>
            </Select>
          ...
          <div className="config-field">
            ...
            <Select value={config.printerConfig.orientation} onValueChange={(value: 'portrait' | 'landscape') => onUpdatePrinterConfig({
            orientation: value
          })}>
              <SelectTrigger className="h-9 text-sm">
          ...
          <div className="config-field">
            ...
            <Input type="text" placeholder="DYMO LabelWriter 450" value={config.printerConfig.printerName} onChange={e => onUpdatePrinterConfig({
        ...
        <TabsContent value="products" className="space-y-4 mt-0">
          ...
          <div>
            ...
            <div className="space-y-2">
              {domesticProducts.map(product => <label key={product.code} className="flex items-center gap-2 cursor-pointer">
                  <Checkbox checked={(config.favoriteProducts || []).includes(product.code)} onCheckedChange={() => toggleFavorite(product.code)} />
          ...
          <div>
            ...
            <div className="space-y-2">
              {internationalProducts.map(product => <label key={product.code} className="flex items-center gap-2 cursor-pointer">
                  <Checkbox checked={(config.favoriteProducts || []).includes(product.code)} onCheckedChange={() => toggleFavorite(product.code)} />
                  <span className="text-sm">{product.name}</span>
                  <span className="text-xs text-muted-foreground">{product.cost.toFixed(2)}€</span>
                </label>)}
